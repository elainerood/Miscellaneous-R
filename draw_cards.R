## Original version from 26 Oct 2018
...
draw_cards <- function(n = 1,
  ...
  #' repl = draw with (TRUE) or without (FALSE) replacement
  #' use_special = use special card names like Ace, Jack, etc (vs 1, 11, etc)
  #' special_cards = designation for special card values (e.g., ace, king, etc).
...
draw_cards2 <- function(n = 1,
  ...
  #' repl = draw with (TRUE) or without (FALSE) replacement
  #' use_special = use special card names like Ace, Jack, etc (vs 1, 11, etc)
  #'   Defaults to not using, because the default settings for a standard card deck
  #'   won't always work for custom decks
  #' special_cards = names/designations for special card values (e.g., ace, king, etc).
  #'   Must be a named character vector where the values are in range 1:per_suit and
  #'   the names are the corresponding designations.
